...
const pageState = reactive({
  ...
  viewType: localStorage.getItem('admin-view-type') || 'table'
})
...
const loadData = async (options = {}) => {
  ...
  try {
    const res = await getAdminPageAPI({
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      const saveAPI = isEdit ? updateAdminAPI : addAdminAPI
      ...
      if (res.code === '200') {
        ElNotification({
          ...
          message: isEdit
            ? `管理员"${formData.username}"已更新`
            : `管理员"${formData.username}"已创建`,
          type: 'success',
          duration: 2000
        })
...
const handleDelete = async (row) => {
  try {
    ...
    try {
      ...
      if (res.code === '200') {
        ElMessage.success(`管理员"${row.username}"已删除`)
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedIds.value.length} 个管理员吗？删除后不可恢复。`,
      '批量删除确认',
      {
        confirmButtonText: '确认删除',
        cancelButtonText: '取消',
    ...
    try {
      const res = await deleteBatchAdminAPI(selectedIds.value)

      if (res.code === '200') {
        ElMessage.success(`已成功删除 ${selectedIds.value.length} 个管理员`)
...
<template>
  <div class="admin-manager">
    ...
    <el-card class="data-card" shadow="hover" :body-style="{ padding: '0px' }" v-loading="pageState.loading"
      ...
      <template v-if="pageState.viewType === 'table'">
        <el-table ref="tableRef" :data="tableData" style="width: 100%" border stripe highlight-current-row
          @selection-change="handleSelectionChange" empty-text="暂无数据" :header-cell-style="{
            ...
            fontWeight: 'bold'
          }" table-layout="auto" class="admin-table" row-key="id">
          <!-- 选择列 -->
          ...
          <el-table-column prop="role" label="角色" width="100" align="center">
            <template #default="{ row }">
              <el-tag type="success">{{ row.role || '管理员' }}</el-tag>
            </template>
          </el-table-column>
      ...
      <template v-else-if="pageState.viewType === 'card'">
        <div class="card-view">
              ...
              <div class="card-content">
                ...
                <div class="info-item">
                  ...
                  <div class="info-value">
                    <el-tag type="success" size="small">{{ item.role || '管理员' }}</el-tag>
                  </div>
                </div>
              </div>
          ...
          <el-empty v-else description="暂无管理员数据" />
        </div>
      </template>
    ...
    <el-dialog v-model="dialogState.visible" :title="dialogState.title" width="500px" append-to-body destroy-on-close
      ...
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" class="admin-form" status-icon>
